...
{
   this(string msg, string file, ulong line) { super(msg, file, line); }
}
...
ubyte[] packLength(ulong l, out uint offset)
{
   ubyte[] t;
   if (!l)
   {
   ...
   this(ubyte* ubp, ulong length)
   {
      ...
      if (*ubp)
      {
         ...
         if (*ubp == cast(ubyte) '#')
         {
            ...
            sqlState[] = (cast(char[])ubp[0LU..5LU])[];
            ubp += 5;
         }
         ulong rem = pe-ubp;
         if (rem)
...
protected:
   ...
   ulong buildAuthPacket()
   {
      ...
      *p++ = 33;
      ...
      foreach (ulong i; 0.._user.length)
         *p++ = _user[i];
      *p++ = 0;
